...
const categories = ["Tout", "Predications", "Enseignements", "Louanges", "Temoignages"];

export function AudioPage() {
  const [search, setSearch] = useState("");
  const [selectedCategory, setSelectedCategory] = useState("Tout");
  const { messages, commentCounts, isLoading } = useMessageList("audio");
  ...
  return (
    <div className="pb-20 lg:pb-6">
      <div className="px-4 lg:px-6 pt-4">
        ...
        <p className="text-[12px] text-muted-foreground mt-4 mb-3">{filtered.length} message{filtered.length > 1 ? "s" : ""} audio</p>
        ...
        {!isLoading && filtered.length === 0 && (
          <div className="text-center py-12">
            <p className="text-muted-foreground text-[14px]">Aucun message trouve</p>
          </div>
        )}
      </div>
    </div>
