...
const categories = ["Tout", "Etudes bibliques", "Meditations", "Articles", "Notes de predication"];

export function TextesPage() {
  const [search, setSearch] = useState("");
  const [selectedCategory, setSelectedCategory] = useState("Tout");
  const { messages, commentCounts, isLoading } = useMessageList("text");
  ...
  return (
    <div className="pb-20 lg:pb-6">
      <div className="px-4 lg:px-6 pt-4">
        ...
        <p className="text-[12px] text-muted-foreground mt-4 mb-3">{filtered.length} texte{filtered.length > 1 ? "s" : ""}</p>
        ...
        {!isLoading && filtered.length === 0 && (
          <div className="text-center py-12">
            <p className="text-muted-foreground text-[14px]">Aucun texte trouve</p>
          </div>
        )}
      </div>
    </div>
