...
import { getVerifiedMfaFactors, requiresMfaStepUp } from "../domain/mfa";
...
export function MfaChallengeDialog() {
  ...
  const needsStepUp = requiresMfaStepUp(isAdmin, mfa);
  const shouldOpen = Boolean(user && needsStepUp && !isDismissed);
  ...
  return (
    <Dialog open={shouldOpen} onOpenChange={(open) => !open && setIsDismissed(true)}>
      <DialogContent className="max-w-md" onPointerDownOutside={(event) => event.preventDefault()}>
        <DialogHeader>
          ...
          <DialogTitle className="text-center">Vérification MFA requise</DialogTitle>
          <DialogDescription className="text-center">
            Confirmez votre identité avec votre application d'authentification avant toute action d'administration.
          </DialogDescription>
        </DialogHeader>
        ...
        {verifiedFactors.length === 0 ? (
          <div className="space-y-4 text-center">
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
              Aucun facteur MFA n'est encore configuré pour ce compte administrateur.
            </div>
            <button type="button" onClick={openSecurityPage} className="w-full rounded-xl bg-[#152a6b] px-4 py-2.5 text-sm font-medium text-white">
              Configurer la sécurité MFA
            </button>
          </div>
        ) : (
